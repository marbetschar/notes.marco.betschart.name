...
# Example ARMA(2,1): Autoregressive moving average of order (2,1)
# X_j - phi_1*X_{j-1} - phi_2*X_{j-2} = W_j + \theta_1 W_{j-1}
...
# one realisation
model = ArmaProcess(ar = np.array([1, -phi_1, -phi_2]), ma = np.array([1, theta_1]))
...
plt.plot(x)
plt.title('Autoregressive Moving Average (ARMA(2,1))')
plt.show()
...
plt.ylabel('$|c_n|^2$')
plt.title('Periodogram of ARMA(2,1)')
plt.show()
...
f = ( 1 + 2*theta_1*np.cos(2*np.pi*s) + theta_1**2 ) / ( 1 - 2*phi_1*(1 - phi_2)*np.cos(2*np.pi*s) - 2*phi_2*np.cos(4*np.pi*s) + phi_1**2 + phi_2**2 )
